...
class Gallery {
    ...
    renderGallery(array) {
        const gallery = array.map(({preview, original, description}) => this.createElement(preview, original, description)).join(""); //прибрали кому при обєднанні масиву
        this.parent.innerHTML = gallery;
    }
    openModal(elem) {
        const instance = basicLightbox.create(`<div class="modal"><img src="${elem.dataset.source}"/></div>`);
        instance.show();
    }
    init() {
